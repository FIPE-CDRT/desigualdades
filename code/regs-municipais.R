...
rais_sp_2019 <- read_csv2(file = 'd:/Users/lucas.dias/Downloads/RAIS_VINC_PUB_SP.txt',
                          ...
                          col_names = TRUE,
                          col_types = NULL) %>%
                dplyr::select(`CBO Ocupação 2002`, `CNAE 2.0 Classe`,
                              `Vínculo Ativo 31/12`, `Qtd Hora Contr`,
                              `Idade`, `Município`, `Raça Cor`,
                              `Vl Remun Dezembro Nom`, `Sexo Trabalhador`)
                dplyr::filter(`Vínculo Ativo 31/12` == 1)
...
raismun_2019 <- rais_sp_2019 %>%
    dplyr::filter(`Vínculo Ativo 31/12` == 1 &
                  ...
                  `CBO Ocupação 2002` != -1) %>%
    dplyr::select(`CBO Ocupação 2002`, `CNAE 2.0 Classe`, `Vínculo Ativo 31/12`,
                  `Qtd Hora Contr`,`Idade`, `Município`, `Raça Cor`,
                  `Vl Remun Dezembro Nom`, `Sexo Trabalhador`) %>%
    dplyr::mutate(negro = ifelse(`Raça Cor` == '04' | `Raça Cor` == '08', 1, 0),
